...
RESET_TMR1 MACRO TMR1_H, TMR1_L ; FUNCIÓN QUE RESETEA EL TMR1 CON CUALQUIER VALOR
    ...
    BCF ((PIR1) and 07Fh), 0 ; LIMPIA LA BANDERA DE INTERRUPCIÓN
    ENDM
...
ISR:
    BTFSC ((PIR1) and 07Fh), 0 ; FUE INTERRUPCIÓN DE TMR1? No=0 Si=1
    CALL INT_TMR1
...
CONFIG_RELOJ:
    ...
    BCF ((OSCCON) and 07Fh), 4 ; PRESCALER = 010 -> 250 kHz
    RETURN
...
CONFIG_TMR0:
    ...
    BCF ((OPTION_REG) and 07Fh), 0 ; PRESCALER 1 : 128
    RESET_TMR0 251
    RETURN

CONFIG_TMR1:
    BANKSEL T1CON
    BCF ((T1CON) and 07Fh), 6 ; EL TMR1 SIEMPRE CUENTA
    BSF ((T1CON) and 07Fh), 5
    BSF ((T1CON) and 07Fh), 4 ; PRESCALER DE 1:8
    BCF ((T1CON) and 07Fh), 3 ; DESHABILITA EL OSCILADOR DE BAJO VOLTAJE
    BCF ((T1CON) and 07Fh), 1 ; RELOJ INTERNO
    BSF ((T1CON) and 07Fh), 0 ; PRENDE EL TMR1
    RESET_TMR1 11100001B, 01111100B ; TMR1 INTERRUPCIÓN CADA 1 SEG
    RETURN

CONFIG_INT:
    BANKSEL PIE1
    BSF ((PIE1) and 07Fh), 0 ; INTERRUPCIÓN TMR1 HABILITADA
    BANKSEL INTCON
    BSF ((INTCON) and 07Fh), 7 ; INTERRUPCIONES GLOBALES HABILITADAS
    BSF ((INTCON) and 07Fh), 6 ; INTERRUPCIONES DE PERIFÉRICOS HABILITADAS
    BCF ((PIR1) and 07Fh), 0 ; LIMPIAR BANDERA DE TMR1
    BSF ((INTCON) and 07Fh), 5 ; INTERRUPCIÓN TMR0 HABILITADA
    BCF ((INTCON) and 07Fh), 2 ; LIMPIAR BANDERA DE TMR0
    RETURN
...
MOSTRAR_VALOR:
 ...
 BCF PORTD, 1 ; APAGA ((PORTD) and 07Fh), 1 Y PRENDE DISPLAY 1
 ...
 MOVF DISPLAY+1, W
 MOVWF PORTB ; COLOCA EL BYTE DE DISPLAY EN PUERTO C
 BCF PORTD, 0 ; APAGA ((PORTD) and 07Fh), 1 Y PRENDE DISPLAY 2
